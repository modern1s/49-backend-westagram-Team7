...
app.get('/users', async(req, res) => {
    ...
    return res.status(500).json({
      "message": error.message  //error라는 객체가 있는건가? 
    })
	}
})
//2. users 생성
...
app.post("/users", async(req, res) => {
    ...
    if (existingUser === req.body.email) { // existing user 이용해서 판별`
...
app.post("/login", async(req, res) => {
  try {
    ...
    // if
    // Password 비교
    // 유저가 입력한 password === DB에서 가져온 PASSfword
    // if 다르면 -> Error
    // 같으면 -> 정상 진행
...
app.delete("/users", async(req, res) => {
  try {const userData = await myDataSource.query(`
  DELETE INTO users (
    name, 
    password,
    email
...
// const payLoader = {id, password:jwt.sign(password)}
// const secretKeyUser = jwt.sign("sdsdsdasda")
// const finalToken =jwt.sign(payLoader, secretKeyUser)

//  getapp("users/" async (req, res)=>{
